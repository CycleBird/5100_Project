            ...
            for i in range(self.game_batch_num):
                ...
                if (i + 1) % self.check_freq == 0:
                    ...
                    current_model_path = os.path.join(
                        self.output_dir,
                        'pruned_current_policy_8_8_4.model'
                    )
                    best_model_path = os.path.join(
                        self.output_dir,
                        'pruned_best_policy_8_8_4.model'
                    )
...
def build_parser():
    parser = argparse.ArgumentParser(
        description='Train pruning-based AlphaZero Gomoku on 8x8x4 and evaluate vs pure MCTS.'
    )
